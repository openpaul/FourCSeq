...
comprises one viewpoint in replicates of 3 conditions.
  ...
  \begin{center}
    \begin{tabular}{ | l | }
      ...
      bioRxiv (2014). doi:10.1101/009548 \cite{Klein2014} \\
      \hline 
    \end{tabular}
  \end{center}
...
\Rclass{FourC} object is created from a \Rclass{SimpleList} \Robject{exptData} 
...
For \Robject{exptData} the following information is required:
\begin{enumerate}
...
Next we create \Robject{exptData} using "exampleData" as directory for the 
...
After creating \Robject{exptData} we now look at \Robject{colData}
...
\begin{enumerate}
  ...
  names in the provided primer file in \Robject{exptData}).
...
<<ObjectInitialization, eval=TRUE, cache=TRUE, dependson=c("filePaths", "exptData", "colData")>>=
...
reference is stored as \Robject{rowData} of the \Rclass{FourC} object.
...
\Robject{rowData} slot with the information on the fragments.
...
fragmentDir folder in the project directory, which are both defined in \Robject{exptData}. 
...
provided in \Robject{exptData}. In a second step this data is loaded by 
...
the provided fragmentDir folder in project directory both defined in \Robject{exptData}.
...
% <<echo=FALSE>>=
% save(fc, 
%      file = file.path(exptData(fc)$projectPath, "fc.rda"),
%      compress="xz")
% @
...
<<>>=
data(fc)
exptData(fc)$projectPath
exptData(fc)$projectPath <- "exampleData"
...
% save(fcf, 
%      file = file.path(exptData(fcf)$projectPath, "fcf.rda"),
%      compress="xz")
% @
...
% <<echo=FALSE, dependson=c("CalculateZScores")>>=
% save(fcf, 
%      file = file.path(exptData(fcf)$projectPath, "fcfDifferences.rda"),
%      compress="xz")
% @
...
the \Robject{FourC} object with \Rfunction{rowData} and find the overlap with
